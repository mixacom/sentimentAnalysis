...
public class DecissionTreeClassification {
	...
	private SplitCondition getBestSplitForScore2() {
		...
		for (double lower=-5; lower<=5; lower+=0.1) {
			for (double upper=lower; upper<=5; upper+=0.1) {
...
//				System.out.println("Progress: " + current.getLower() + ", " + current.getUpper());
	...
	public static void main(String[] args) {
		try {
	        ArrayList<Tweet> annotatedSet = readTweetsFromCSV("/Users/wilcovanleeuwen/Github/sentimentAnalysis/src/main/java/classification/testSetManyWP.csv");
	       ...
	       for (int i=0 ; i<annotatedSet.size(); i++) {
			...
			System.out.println(posAnnotated.size());
			System.out.println(negAnnotated.size());
...
//        }
	...
	private static void writeScore(String fileNameWrite, String fileNameRead) throws IOException {
		...
		while ((nextLine = testSetFile.readNext()) != null) {
			if (nextLine[0] != null) {
				double score = s.calculate(nextLine[0]);
